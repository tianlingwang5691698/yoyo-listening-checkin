...
async function callYoyo(action, payload) {
  ...
  try {
    let timeoutMs = 12000;
    if (action === 'analyzeWritingTranslation' || action === 'submitWritingAttempt' || action === 'gradeWritingAttempt' || action === 'generateWritingBandSample' || action === 'getWritingAttemptDetail') {
      timeoutMs = 320000;
