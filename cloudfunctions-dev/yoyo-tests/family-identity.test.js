...
test('getTaskDetail lesson view 返回精简 categoryTasks', async (t) => {
  t.mock.method(studyFacade, 'prepareRequestContext', async () => ({
    ctx: {
      user: { userId: 'user-1' },
      member: { memberId: 'member-1', studyRole: 'parent' },
  ...
  assert.deepEqual(result.categoryTasks[0], {
    category: 'peppa',
    taskId: 'peppa-1',
    title: 'Peppa',
    displayTitle: 'Peppa',
  ...
  assert.equal(result.categoryTasks[0].audioUrl, undefined);
  assert.equal(result.categoryTasks[0].transcriptTrackId, undefined);
});
