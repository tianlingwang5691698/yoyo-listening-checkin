...
- 既有孩子档案在读取时自动补齐 6 位学号。
- 新增 `joinFamilyByChildCode`，家长输入学号后加入孩子所在家庭。
- 我的页重构为身份页：学号、昵称编辑、每日一句、日报入口、家庭管理入口。
- 身份页接入真实绑定：家长输入学号，学生查看并复制自己的学号。
- 家庭页把学号作为主绑定入口，旧邀请码降级为兼容入口。
...
- 我的页从多面板说明式结构收敛为：顶部身份、学号、轻量操作列表。
- 移除学号的 `生成中` 文案，改为 `同步中 / 待同步`。
- 学号未就绪时隐藏复制按钮，避免无效操作。
- 昵称、日报、家庭、绑定改为轻列表，减少说明句和按钮堆叠。

扫描结果：

- `pages/profile/index.js` 语法检查通过。
- 我的页未发现 `生成中`、说明式长句和旧入口文案。
- 页面层未发现 `控制台`、`Dashboard`、`Console`、`PROGRAM CONSOLE`、`报表式总览`、`歌曲`。
- 页面层未发现 `>开始<` 和 `0/1`。

## 2026-04-19 我的页去重与操作对齐

整改内容：

- 页面内移除重复的 `我的` 标题，仅保留顶部导航中的 `我的`。
- 顶部居中展示孩子昵称和每日一句。
- 学号放到昵称编辑下方的统一列表里，移除独立学号卡和 `可绑定` 状态。
- 家庭行移除重复家庭名，仅保留连接状态。
- 昵称保存按钮改为圆形图标按钮，解决胶囊按钮视觉不对称。
- 删除我的页 `绑定` 行，绑定入口由家庭页和身份页承载。
- 页面列表只保留昵称、ID、日报、家庭四个不同职责。

扫描结果：

- `pages/profile/index.js` 语法检查通过。
- 我的页未发现 `生成中`、重复 `我的`、`可绑定`、说明式长句和旧入口文案。
- 页面层未发现 `控制台`、`Dashboard`、`Console`、`PROGRAM CONSOLE`、`报表式总览`、`歌曲`。
- 页面层未发现 `>开始<` 和 `0/1`。
...
- 我的页保存和复制按钮统一为同尺寸文字按钮。
- 我的页保存和复制按钮使用统一 `action-row` 对齐规则，对齐到输入/数值行。
- 家庭页只保留学号、加入记录、成员列表。
- 家庭页删除昵称编辑、订阅状态、当前身份、兼容邀请码页面入口和重复家庭信息。
- 日报页只保留今日明细和最近 7 天。
- 日报页删除成员模块、订阅按钮、家庭名/孩子名组合说明和统计大卡。
- 家庭页 ID 复制和加入确认使用同款文字按钮。
- 家庭页复制和加入按钮使用统一 `action-row` 对齐规则。

扫描结果：

- `pages/profile/index.js`、`pages/family/index.js`、`pages/parent/index.js` 语法检查通过。
- 家庭页页面层未发现邀请码主入口、订阅、当前身份、孩子档案等重复模块。
- 日报页页面层未发现家庭成员模块和订阅按钮。
- 页面操作按钮未发现 `✓`、`⧉` 图标残留。
- 页面层未发现 `控制台`、`Dashboard`、`Console`、`PROGRAM CONSOLE`、`报表式总览`、`歌曲`。
- 页面层未发现 `>开始<` 和 `0/1`。
