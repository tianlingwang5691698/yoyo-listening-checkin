...
- 高中阅读逐题解析按稳定 `passageId` 全局复用；云端生成任务必须持久化防重，每批模型结果即时保存。学生退出、断线或重新进入时只轮询同一任务并读取共享结果，恢复时仅补缺失题号，不重新调用整篇解析。
...
- `Grammar and Vocabulary Section A` 每个真实题号必须恰好对应一个文内输入框；OCR 丢失下划线时只允许依据同题号和紧邻括号提示词恢复。重复题号不得把 `30 seconds` 等正文数字误作空位，错位标记必须在生成器、正式 JSON 和旧线上展示兼容中同时修复。
...
2. 通过 JSON/ID/题号/答案/音频规格校验，并确认 `正文污染=0`、`标题进入正文=0`、`空段落=0`、`题干选项重复=0`、`文内空位错位=0`、`每题可作答空位=1`；再运行 `node --check`、回归测试和增量上传 dry-run。
