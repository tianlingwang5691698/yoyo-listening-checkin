...
- 线上用户只能看到内容增加，不能因为本次上传少识别内容而减少。
- 上传前必须拉取线上当前 JSON 备份；本地生成结果只作为新增候选。
- 合并键固定优先用 `_id`；旧 `_id` 必须全部保留。
- 旧记录的字段、题目、答案、图片路径、音频路径默认不改。
- 新记录必须用新的稳定 `_id`；新音频/图片/transcript 文件名不得复用旧文件名。
- 上传后必须校验：总数不小于上传前、旧 `_id` 全存在、目标 HTTPS 返回 200。
- 不得为了新前端改动破坏旧前端和原云函数可用性；新增接口只能作为补充，原接口返回结构必须向后兼容。
- 最终上传的数据结构、路径和字段必须与线上存储现状一致兼容；不能要求修改云函数或前端才能可用，不能影响线上上一个版本用户。

如果必须修复旧记录：

- 先备份线上旧 JSON。
- 单独列出受影响 `_id`、旧值、新值和原因。
- 只改明确错误字段，不能顺手重排、重命名或重建全量内容。
...
- C 篇首字母填空中，正文空位数量与题目数量一致且首字母逐项对应时，正文重复题号、缺号或整组偏移必须按 `questions[]` 顺序恢复；数量或首字母不一致时拒绝猜测。门禁要求每个题号恰好生成一个输入框。
