...
### 2026-07-27 已完成跟读无法进入与今日目标显示 25 分钟

1. 现象：317613 当天已完成的跟读段落点击后无法进入；首页“今日听力”目标显示旧值 25 分钟。
2. 查询：当天口语段落均为已完成；当前固定听力计划实际目标约 41 分钟，旧日报任务与当前计划不一致。
3. 结论：阶段页防重复逻辑把已完成段落直接拦截；首页固定计划无条件信任当天旧日报分钟。
4. 修复：已完成段进入回看模式并恢复每句成绩/录音；未完成段继续按唯一句子 ID 防重。日报任务签名与当前计划不一致时，首页按当前任务计算分钟。
5. 是否需要发版：需部署 `yoyo` 云函数，并重新上传发布小程序前端。
...
2. 查询：`studyCompletedItems.taskId=unlock1workbook-3`；实际缓存 `listeningStudyPacks.listeningId=lesson-unlock1workbook-unlock1workbook-3__fixed_listening_round_1`，三类学习卡完整。
3. 结论：固定计划生成缓存使用带轮次任务 ID，课程详情完成记录被公共目录基础 ID 覆盖，日报精确缓存查询未命中后将记录过滤。
4. 修复：精确 ID 未命中时按同一音频标题回查现有学习包缓存，不修改学习进度或历史完成记录。
5. 是否需要发版：需部署 `yoyo` 云函数；前端无需重新发布。

### 2026-07-24 佑佑语法词法第二轮与句法接续

1. 规则：当前词法第一轮按真实完成进度每日 5 课；168 课完成后从名词第 1 课开始第二轮，每日 10 课；第二轮结束后进入句法，每日 5 课。
2. 隔离：词法第二轮使用独立任务 ID，不能复用第一轮完成记录；句法使用正式 5 个专题共 106 课。
3. 回归：第一轮完成后返回名词前 10 课；第二轮只跳过实际完成项；第二轮全部完成后返回句法前 5 课；句法徽标显示“句法”。
4. 是否需要发版：需部署 `yoyo` 云函数；计划概览文案需重新发布小程序前端。

### 2026-07-24 Unlock 1 练习册结束后进入 Unlock 2

1. 规则：佑佑完成 Unlock 1 练习册第三轮后，下一学习日进入 Unlock 2 听口课本。
2. 推进：首轮每日 1 集、每集 3 遍；首轮结束后每日 3 集、每集 1 遍，按正式目录长期循环，各槽位独立推进。
3. 回归：第三轮最后 3 集完成后返回 Unlock 2 第 1 集；第 1 集完成 3 遍后返回第 2 集；首轮全部完成后返回第 1–3 集各 1 遍，仅完成第 1 槽位后返回第 4、2、3 集。
4. 是否需要发版：需部署 `yoyo` 云函数。

### 2026-07-24 佑佑新概念 1 结束后进入新概念 2

1. 现象：317613 的新概念 1 当前轮只剩 139–144，原逻辑结束后会循环回新概念 1 开头。
2. 查询：新概念 1 正式目录 72 组；新概念 2 正式 96 课，本地目录 98 条中第 5、21 课各有 1 条时间戳重复音频。
3. 修复：新概念 1 各槽位本轮全部完成后，下一学习日切换到新概念 2；按课号去重为 96 课。首轮每日 1 课、每课 3 遍；首轮结束后每日 3 课、每课 1 遍循环。
4. 回归：完成新概念 1 最后 3 组后返回新概念 2 第 1 课；第 1 课完成 3 遍后次日返回第 2 课；第 5、21 课不重复排入；首轮 96 课完成后返回第 1–3 课各 1 遍，32 日后开始下一循环。
5. 是否需要发版：需部署 `yoyo` 云函数。

### 2026-07-24 佑佑 Unlock 1 练习册完成后未推进

1. 现象：317613 在 2026-07-23 完成练习册第二版 1.1，2026-07-24 仍推荐 1.1。
2. 查询：`dailyTaskProgress` 中该记录为 `3/3 + completedToday=true`，但 `taskId=unlock1workbook-1` 且 `planSlotIndex=0`；`fixedPlanProgressSummaries` 无 `unlock1workbook__1`。
3. 结论：`markTaskListened` 先命中独立课程目录，绕过佑佑固定计划，完成记录丢失槽位。
4. 修复：佑佑 `normal` 固定计划在详情、完成写入和连续播放队列中均优先于独立课程目录；每个任务完成后按自身槽位推进。
5. 回归：完成 `unlock1workbook-1__fixed_listening_round_1` 后必须写入 `planSlotIndex=1`，下一学习日推荐 2.1，不读独立目录任务。
6. 数据校正：已将 2026-07-23 的 1.1 完成记录恢复为固定计划槽位 1，重建 `fixedPlanProgressSummaries`；线上计算 2026-07-25 下一任务为 2.1。
7. 是否需要发版：已部署 `yoyo` 云函数。

### 2026-07-24 真实阅读记录 PDF 模型成功但页面显示失败

1. 现象：练习记录下载阅读 PDF 时模型已连续返回学习包内容，页面仍显示生成失败。
2. 查询：11:05:10 请求运行 24.071 秒后报 `reading-study-pack-missing-phrases`；11:05:51 请求运行 90.114 秒并成功上传 `2020 上海松江一模阅读 D.pdf`，但客户端资源连接先超时；11:09:10 重试 4.68 秒成功返回同一路径。
3. 结论：整包卡片缺少短语时有效分区未立即共享持久化；长请求虽在云端完成，客户端没有成品续取机制，因此把实际成功显示为失败。
4. 修复：学习卡按 `passageId` 确定性合并，整包返回不完整时只补缺失分区并逐项保存；PDF 使用稳定路径缓存，客户端超时后仅轮询成品，生成完成自动打开，再次点击直接返回。
5. 是否需要发版：需部署 `yoyo` 云函数，并重新发布小程序前端。
...
2. 查询：`buildStudyPackWithModel -> normalizeStudyPack -> validateQuestionStudyPack`；模型整包返回缺少真实题号 4。
3. 结论：接口调用成功，失败发生在返回结构完整性校验；不完整结果未写入共享缓存。
4. 修复：首次仍生成整套逐题解析，校验后仅按真实题号补请求缺失题并重新合并；最终答案强制使用题库标准答案，完整复验后才保存。
5. 是否需要发版：需部署 `yoyo` 云函数；前端无需重新发布。
...
### 2026-07-23 家长 IELTS 写作长时间批改且结果虚高

1. 现象：家长 Task 1 已产生模型费用，页面仍显示“批改中”，最终四项均为 8。
2. 查询：`writingPreviewAttempts/4a3a61cc9520277b191552e8981b33b8` 已为 `graded`，但残留 `gradeError=writing-timeout`；模型逐档结论没有逐句证明 Band 8 全部官方正向特征。
3. 结论：170 秒轮询阈值早于正常长评分完成时间，详情查询重复发起模型；评分结构只检查笼统证据，模型可把“基本符合”写成“全部满足”。
4. 修复：正常 `grading` 330 秒内只读轮询，成功写回清除旧错误；所授档位每条官方英文正向特征必须分别提供直接原文证据，缺一即重新校准。
5. 是否需要发版：需要部署 `yoyo` 云函数，并重新发布小程序前端。

### 2026-07-23 Terra 内容评分试用

1. 范围：写作评分、阅读/听力学习解析、语法讲解、口语内容评分。
2. 云环境：主模型和内容备用模型统一为 `gpt-5.6-terra`；口语禁止备用回退。
3. 缓存：已有阅读、听力、语法共享结果和历史评分继续复用；仅新产生且缓存未命中的内容调用 Terra。
...
### 2026-07-24 Cambridge 19 Test 1 写作误报超时

1. 现象：页面显示“批改超时”，模型实际已成功返回；重试后获得 Band 7.5。
2. 记录：`writingPreviewAttempts/98c101f2898030d71c62cebfb25f84a6`，最终状态 `graded`，四项为 `7/8/7/7`。
3. 结论：首次请求不是模型超时，而是 `writing-ielts-official-decision-invalid`；前端把所有 `grading-failed` 误显示为超时。
4. 修复：校准超时或逐档证据不完整时保留有效四项评分；前端按超时、结构不完整和普通失败分别提示。
5. 是否需要发版：需部署 `yoyo` 云函数并重新发布小程序。
6. 评分复核：原结果 `TA 7 / CC 8 / LR 7 / GRA 7 = 7.5` 中，Terra 将常规连接词和基础趋势词误作 LR 7 证据，并将简单并列及单个关系从句误作 GRA 7 的复杂结构多样性；新增 `3.5–7.5` 独立盲校准，评分版本升级后重算同文。
7. 线上复测：同一篇 150 词原文生成新预览任务 `preview-257ab605a3bcd030c8b384ba76901031`，首轮与独立校准均得到 `TA 7 / CC 7 / LR 6 / GRA 6 = 6.5`，状态 `graded`，客户端评分连接超时后仍成功轮询取回。
