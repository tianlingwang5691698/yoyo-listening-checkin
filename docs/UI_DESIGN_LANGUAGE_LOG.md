...
- 家长/老师模式可绑定多个学生 ID，并在绑定学生列表中切换查看对象；切换只影响查看 target，不改变本机学生账号。
...
- 改动：录音前优先检查微信 App 的系统麦克风授权；系统层拒绝时引导去系统授权页，文案明确为本机系统麦克风权限；系统可用后再触发小程序录音授权；录音启动前暂停课程音频，失败时在回答评分区写出 `errMsg/systemMicrophone/scope.record/audioWasPlaying`。
- 设计记录：真实用户优先处理 iPhone、安卓手机、Windows 或 macOS 的本机/微信麦克风权限；若本机权限可用但小程序录音 scope 被拒绝，再按微信要求处理小程序录音授权；测试期不能只显示泛化 toast。
- 验证：已做脚本语法检查。
...
- 设计记录：家长绑定目标用于跟踪查看，后台默认聚焦被绑定学生，避免把家长自己的本机学生 ID 混入管理列表。
- 验证：已做云函数脚本语法检查。
...
- 文件：`cloudfunctions/yoyo/services/admin.service.js`
- 文件：`pages/admin/index.wxml`
- 改动：活跃用户如果绑定过其他学生 ID，列表身份显示为家长；否则显示为学生。
...
- 文件：`pages/admin/index.wxml`
- 文件：`pages/admin/index.wxss`
- 改动：学生卡片展开后只显示学生、学生 ID、家长姓名；前端不再展示学生设备、本机学生、userId、openId、家庭 ID、诊断信息或详情入口，云端成员信息也只返回姓名和角色。
...
- 文件：`pages/admin/index.wxml`
- 改动：学生昵称已在卡片标题显示，展开区移除重复的“学生：昵称”行，只保留学生 ID 和家长。
...
- 文件：`cloudfunctions/yoyo/services/admin.service.js`
- 文件：`pages/admin/index.wxml`
- 改动：展开区“学生 ID”简化为“ID”；家长绑定判断改为 role=parent 且不是“学生设备”占位，确保有绑定时展开区显示家长昵称。
...
- 改动：家长绑定孩子 ID 时如果未填写备注，默认使用本机 owner 学生昵称作为家长昵称；本机昵称仍为占位时拒绝绑定并提示先设置昵称。
...
- 文件：`pages/admin/index.wxml`
- 文件：`pages/admin/index.wxss`
- 改动：我的页头像区点击 1 次后尝试进入管理员后台；后台页只展示学生 ID、学生昵称和成员绑定关系。
...
- 设计记录：家长/老师解绑再绑定同一学生后，学习记录仍属于该学生 ID；记录查询不能回落到本机学生 ID。
- 验证：语法检查通过。
