...
- 改动：录音入口不再主动查询本机或小程序麦克风权限；点击录音时先走微信隐私接口授权 `wx.requirePrivacyAuthorize`，通过后暂停课程音频并启动录音；保留失败 `errMsg/audioWasPlaying` 调试信息。
- 设计记录：`operateRecorder:fail appid privacy api banned` 属于微信隐私接口拦截，不是麦克风权限；录音入口只补隐私授权前置，不恢复权限查询和设置页跳转。
- 验证：已做脚本语法检查。
