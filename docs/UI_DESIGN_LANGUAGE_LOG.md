...
- 改动：录音入口不再主动查询本机或小程序权限，点击后直接暂停课程音频并启动录音；移除权限查询和设置页跳转，保留失败 `errMsg/audioWasPlaying` 调试信息。
- 设计记录：线上用户通常已允许微信麦克风，录音入口避免额外权限查询和设置页跳转，把授权交给微信原生录音启动流程处理；测试期保留录音失败断点。
- 验证：已做脚本语法检查。
