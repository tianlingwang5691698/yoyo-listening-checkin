...
适用：听力、阅读、语法、写作、词汇、transcript 和所有线上 `_content/` 上传内容。

- 内容上传必须增量合并，不能全量覆盖导致线上内容减少。
- 已上线记录的 `_id` 是稳定主键，不得重命名或删除。
- 已上线文件路径是稳定引用，不得复用旧路径上传不同内容。
- 新增字段必须可选，旧前端和原云函数读不到新字段也必须可用。
- 旧接口返回结构必须向后兼容；新增接口只能补充能力，不能让旧版本失效。
- 最终上传的数据必须沿用线上存储现有结构、路径和字段，不得要求云函数或前端配套修改才能使用。
- 线上上一个版本用户必须继续看到原有内容并可正常进入。
- 上传验证必须包含旧内容仍可见、新内容新增可见、总数不下降。
