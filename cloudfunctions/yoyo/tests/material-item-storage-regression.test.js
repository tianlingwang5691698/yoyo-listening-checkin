...
test('听力与写作目录使用轻量摘要且详情优先按 id 读取单条文件', () => {
  ...
  assert.equal(directory.listeningSeniorSpring.length, 8);
  assert.equal(directory.listeningSeniorAutumn.length, 10);
  assert.equal(directory.writingSeniorSpring.length, 29);
  ...
  assert.equal(directory.writingSeniorAutumn.length, 31);
  ...
  assert.match(storeSource, /catalogVersion: 'senior-2026-v3'/);
  assert.match(materialPageSource, /materialHomeSnapshotV4/);
