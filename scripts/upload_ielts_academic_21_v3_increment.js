...
const LOCAL_ROOT = path.join(ROOT, 'data', 'ielts-academic', 'cambridge-21', 'writing');
const CLOUD_ROOT = '_content/ielts-academic/cambridge-21/writing';
const REPORT_PATH = path.join(LOCAL_ROOT, 'upload-v3-report.json');
...
async function main() {
  ...
  const entries = [];
  addDirectory(entries, path.join(LOCAL_ROOT, 'items-v3'), `${CLOUD_ROOT}/items-v3`, '.json');
  addDirectory(entries, path.join(LOCAL_ROOT, 'visuals-v3'), `${CLOUD_ROOT}/visuals-v3`, '.jpg');
  ...
  if (apply) {
    ...
    await mapLimit(entries, 8, async (entry) => {
      const check = await request(entry.cloudPath);
      if (check.statusCode !== 200 || sha1(check.body) !== entry.sha1) report.missingAfterUpload.push(entry.cloudPath);
    });
  }
