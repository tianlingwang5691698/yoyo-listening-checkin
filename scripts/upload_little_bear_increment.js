...
const BUILD_ROOT = path.join(ROOT, 'data', 'transcript-build', 'little-bear', 'Pre A1', 'little-bear');
...
const REFERENCE_PATH = '_transcripts/A1/songs/bundle.json';
...
async function main() {
  ...
  if (manifest.meta.level !== 'Pre A1' || manifest.tracks.length !== 17 || Object.keys(bundle).length !== 17) {
    throw new Error('expected 17 safe Pre A1 Little Bear tracks');
  }
  if (cleanReport.validationErrorCount !== 0 || cleanReport.readyTrackCount !== 17 || cleanReport.rejectedMappingCount !== 7) {
    throw new Error('local Little Bear validation report is not safe to upload');
  }
  ...
  const transcriptItem = {
    type: 'transcript',
    title: 'Little Bear sentence bundle',
  ...
  if (items.some((item) => !fs.existsSync(item.localPath))) throw new Error('Little Bear local upload file missing');
  if (items.some((item) => normalizeCloudPath(item.cloudPath).split('/').some((part) => !part || part !== part.trim()))) {
    throw new Error('Little Bear cloud path invalid');
  }
  ...
  const preflight = {
    ...
    rejectedSourceCount: cleanReport.rejectedMappingCount,
  ...
  const sampleItems = [audioItems[0], audioItems[Math.floor(audioItems.length / 2)], audioItems.at(-1), transcriptItem];
  const hashVerify = [];
