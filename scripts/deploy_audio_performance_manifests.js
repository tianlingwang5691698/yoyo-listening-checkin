...
const REPORT_PATH = path.join(ROOT, 'data', 'transcript-build', 'audio-performance', 'deploy-report.json');
const FILES = [
...
async function main() {
  ...
  FILES.forEach((file) => {
    if (!fs.existsSync(path.join(FUNCTION_ROOT, 'yoyo', file))) throw new Error(`missing deploy file: ${file}`);
  });
  const manager = new CloudBaseManager({ ...credentials(), envId: appConfig.cloudEnvId });
  const before = await waitActive(manager);
  if (!apply) {
    console.log(JSON.stringify({ mode: 'dry-run', statusBefore: before.Status, files: FILES }, null, 2));
    return;
  }
  const requests = [];
