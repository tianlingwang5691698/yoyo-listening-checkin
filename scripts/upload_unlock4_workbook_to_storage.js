...
const BUILD_ROOT = path.join(ROOT, 'data', 'transcript-build', 'unlock4-workbook', 'B2', 'unlock4');
...
async function main() {
  ...
  const transcriptItem = {
    type: 'transcript',
    title: 'unlock4 workbook sentence bundle',
  ...
  if (!apply) {
    console.log(JSON.stringify({
      ...
      transcriptCount: 1,
      transcriptPath,
      sample: checks.slice(0, 6).map((item) => ({
        type: item.type,
        cloudPath: item.cloudPath,
