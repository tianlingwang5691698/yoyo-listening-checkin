...
const RELEASE = '20260727-subpages-v1';
const NAMES = [
  'profile-operator.jpg',
  'listening-comms.jpg',
  'reading-intel.jpg',
  'grammar-briefing.jpg',
  'writing-field-notes.jpg',
  'speaking-radio.jpg',
  'vocabulary-inventory.jpg',
  'growth-after-action.jpg'
];
const ASSETS = NAMES.map((name) => ({
  name,
  localPath: path.join(ROOT, 'assets', 'tactical', 'subpages-v1', name),
  cloudPath: `_assets/themes/tactical/${RELEASE}/${name}`
}));
...
async function main() {
  ...
  for (const { asset, status } of statuses) {
    ...
    results.push({ name: asset.name, cloudPath: asset.cloudPath, url: publicUrl(asset.cloudPath), status: finalStatus });
  }
