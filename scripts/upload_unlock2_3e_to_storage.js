...
const BUILD_ROOT = path.join(ROOT, 'data', 'transcript-build', 'unlock2-third-edition', 'A2', 'unlock2');
...
const oldBundleBackupPath = path.join(BUILD_ROOT, 'cloud-backup', 'unlock2-bundle-before-3e-upload-20260711.json');
const credentialPath = path.join(ROOT, 'SecretKey.csv');
const oldBundleCloudPath = '_transcripts/A2/unlock2/bundle.json';
...
async function main() {
  ...
  if (manifest.tracks.length !== 63 || Object.keys(bundle).length !== 63) {
    throw new Error('expected 63 local audio and transcript tracks');
  }
  if (cleanReport.validationErrorCount !== 0 || cleanReport.missingTextCount !== 0) {
    throw new Error('local bundle validation has blocking errors');
  }
  if (!fs.existsSync(oldBundleBackupPath)) {
    throw new Error('old Unlock 2 cloud bundle backup is missing');
  }

  const oldBackup = fs.readFileSync(oldBundleBackupPath);
  JSON.parse(oldBackup.toString('utf8'));
  const oldCloud = await requestBuffer(oldBundleCloudPath);
  if (oldCloud.status !== 200 || sha1Buffer(oldCloud.body) !== sha1Buffer(oldBackup)) {
    throw new Error('old Unlock 2 cloud bundle changed after backup; pull a new backup before upload');
  }
  ...
  const transcriptItem = {
    type: 'transcript',
    title: 'Unlock 2 Listening Speaking Third Edition sentence bundle',
  ...
  if (badPaths.length) {
    throw new Error(`cloud path has leading/trailing spaces: ${badPaths.map((item) => item.cloudPath).join(' | ')}`);
  }
  ...
  if (Object.keys(transcriptData).length !== 63 || !oldBundleUnchanged) {
