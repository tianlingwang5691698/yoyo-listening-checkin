...
const INFORMATION_ORDER_NARRATION_TEXTS={
 'information-order-essence':`Mia solved the problem. <#0.7#>最普通的主动顺序，先说米娅，再说她解决了问题。The problem was solved by Mia. <#0.7#>事实没有变化，但被动句把问题放到前面，更适合承接上文正在谈的问题。That the plan failed is obvious. <#0.7#>这句语法正确，只是很长的主语压在开头，听起来有些头重。Only then did we understand the cause. <#0.8#>这里把“直到那时”推到最前，注意力立刻落到时间边界上，还带出了倒装。
信息顺序可以理解成给读者安排一条阅读路线。语法先决定哪些结构能成立，信息顺序再决定从哪里起步、重点落在哪里。普通表达通常先放已经知道或较短的内容，再送出新的、较长的内容。需要承接旧话题，可以换成被动；内容太长，可以借形式成分把它放后面；确实需要对比，才使用前置或倒装。<#0.8#>判断时先确认谁做什么的事实关系没有变，再问三个问题：上文正在谈谁，哪部分是新消息，哪部分最值得强调。顺序服务的是理解，不是随意移动单词。`,
 'skeleton-vs-topic':`This problem, we need to discuss. <#0.7#>问题被提前了，但它在结构上仍然是“讨论”的对象。这种说法需要前文承接或明显对比，否则会显得刻意。We need to discuss this problem. <#0.7#>这里先交代“我们需要讨论”，再放讨论对象，是普通陈述最稳妥的主干。As for this problem, we need to discuss it further. <#0.8#>开头用明确标记引出话题，主句里又用 it 接住，所以结构完整。
中文很容易先抛出一个话题，再补一句评论；英语不是不能这样做，而是要先保证主句有清楚的主语和谓语。把“这个问题”放最前时，要知道它不是自动变成主语，真正的动作发出者还是 we。<#0.7#>写句子时先问：谁需要做什么？搭出主干后，再决定是否值得把某个对象提到前面。没有特殊上下文，就把宾语留在动词后；真要突出话题，就用清楚的引导方式，并让主句内部仍能指回它。这样既保留中文想强调的内容，也不会留下悬空话题。`,
 'given-new-flow':`I bought a new camera yesterday. The camera takes excellent photos. <#0.7#>第一句把一台新相机带进谈话，第二句改用 the camera，告诉读者还是刚才那个对象，然后补上拍照效果这个新消息。A student was waiting outside. <#0.7#>这里第一次出现某个学生，用不定表达很自然，因为听者还不知道是谁。The main problem is the lack of reliable data. <#0.8#>句首先放已经确定的“主要问题”，句尾才揭晓真正要强调的新内容：缺少可靠数据。
顺畅的信息流像递东西：上一句刚交给读者的内容，下一句可以拿它当起点，再交出下一条新消息。第一次出现的对象通常是新的；再次提到时，就要用读者能认出的方式承接。<#0.7#>新信息也常落在句尾，因为句尾最容易形成记忆重点。判断时不要只看冠词或位置，要联系上下文：读者已经知道什么，现在最需要知道什么？让旧信息负责搭桥，让新信息负责推进，句子之间就不会突然跳开。`,
 'end-weight-short-long':`It is important that every student understands the rule. <#0.7#>先给出“很重要”这个判断，再把较长的真正内容放后面，听者不用一开始就背着一大串信息。She gave him a detailed explanation of the new policy. <#0.7#>较短的接收者 him 靠前，较长的事物内容放后，读起来更平衡。We explained the problem to the students who had just arrived. <#0.8#>先说较短的 problem，再把带有后置说明的学生群体放到句尾。
尾重不是死规定，而是照顾理解速度：短、熟悉、容易处理的成分先出现，长、复杂、新出现的成分后出现。这样读者先抓住主干，再展开细节。<#0.7#>遇到长从句作主语，可以用形式主语把真正内容后移；遇到两个宾语，可以比较哪一部分更长、更需要补充说明；遇到带从句的名词短语，也尽量别让它挡住主干。检查时读一遍，看看是否要等很久才能听见核心动词。只要事实关系不变，适当调整重量，句子会明显更自然。`,
 'dummy-subject':`It is obvious that we need more time. <#0.7#>it 先占住主语位置，让“很明显”尽快出现，真正明显的内容放在后面的从句里。It took us two hours to finish the task. <#0.7#>这里先说花费，再交代所需时间和真正完成的事情，信息一层层展开。It seems that the plan will work. <#0.8#>it 不指某个具体东西，而是帮助“看来”这个判断先落地。
形式主语可以把它理解成一个临时座位。英语句子需要主语，但真正主语如果是一大段从句或不定式，直接堵在开头会很重，于是先让 it 坐在主语位，再把具体内容安排到后面。<#0.7#>判断它是不是形式主语，不能翻译成“它”就结束，而要问：句中真正被认为明显、花费时间或看起来怎样的内容是什么？找到后面的长内容，才能看清结构。使用时也别见到 it 就后移，只有当后面内容才是真正核心，而且前轻后重更自然时，这个安排才有价值。`,
 'dummy-object':`We find it difficult to solve the problem. <#0.7#>find 后面的 it 先占宾语位置，difficult 紧跟着说明评价，真正觉得困难的事情放在最后。She made it clear that she would leave. <#0.7#>clear 需要靠近宾语，后面的从句才是被说明清楚的具体内容。I consider it a mistake to ignore the warning. <#0.8#>a mistake 是对宾语的判断，真正被认为是错误的，是忽视警告这件事。
形式宾语解决的是一个位置冲突：动词后既要接宾语，又要马上接一个说明宾语怎样的补语；可真正宾语偏偏很长。于是 it 先占位，让补语贴近它，长内容再后置。<#0.7#>分析时按三步走：先找动词，接着找 it 后面对它作出的评价，最后找真正承受这项评价的从句或不定式。不要把 it 当成具体事物，也不要漏掉后面的真正宾语。只要看清“占位、评价、真内容”这三层，句子就不会拆乱。`,
 'existential-new-information':`There is a message for you. <#0.7#>句子先告诉你“有”，再引出一条新消息，重点不是 there 这个地点。There are two problems with this plan. <#0.7#>这次新出现的是两个问题，数量是复数，所以动词也跟着变化。A tall building stands near the station. <#0.8#>这里直接拿高楼作主语，是在描述一个已经能够作为话题的对象，而不是单纯把它介绍进场景。
存在句像把镜头先对准一个场景，再让新对象进入画面。there 主要负责开启结构，真正需要关注的是后面的名词短语。<#0.7#>第一次告诉别人某处有某物时，这种顺序很自然；如果对象已经是谈话中心，就更适合直接让它作主语。还要注意，存在句中的动词形式通常看后面的名词是单数还是复数。判断时问：我是在介绍一个新对象，还是在继续描述一个已知对象？前者适合存在句，后者常用普通主语句。不要把所有“在某处”都机械写成 there。`,
 'basic-adverbial-position':`She spoke quietly in the library. <#0.7#>方式 quietly 紧跟动作，地点放后面，先回答怎么说，再回答在哪里说。We met at the station yesterday. <#0.7#>地点和时间都放在句尾，普通顺序先给地点，再给较大的时间背景。Yesterday, we met at the station. <#0.8#>时间被移到句首，是先搭出昨天这个背景，主干仍保持完整。
时间、地点、方式状语都在补充动作背景，但位置不是完全自由。中性表达里，方式通常靠近动词，地点在其后，时间往往更靠后；如果时间负责承接上文或切换场景，也可以放句首。<#0.7#>判断时先锁定它修饰哪个动作，再看它只是普通补充，还是要负责搭背景。普通补充用常规位置最清楚，需要连接上下文才前置。移动以后还要检查：有没有把状语误放到另一个动词旁边，造成修饰对象不明。位置选择的目标是让读者一次看懂“怎样、哪里、什么时候”。`,
 'frequency-adverbs':`She usually reads at night. <#0.7#>usually 放在实义动词 reads 前，表示这个动作通常发生。He is often late. <#0.7#>遇到 be 动词，often 放在它后面，说明迟到的频率。They have never seen snow. <#0.8#>谓语里有助动词 have，never 放在第一个助动词后、主要动词前。
频率副词的位置看起来有三套，其实只是在找谓语的支点。普通实义动词自己承担动作，频率词放它前面；be 本身很显眼，频率词跟在后面；谓语由助动词加主要动词组成时，频率词进入两者之间。<#0.7#>检查时先圈出完整谓语，再判断有没有 be 或助动词，不要只凭中文顺序搬动。句首也可能出现某些频率表达，但会带来更强的话题或对比效果。没有特殊强调，就优先使用中性位置，让频率和它真正限制的动作紧密相连。`,
 'multiple-adverbials':`She worked carefully in the lab all morning. <#0.7#>先说工作得怎样，再说在哪里，最后给出整段时间，信息从动作内部逐步扩展到大背景。Every morning, he quietly reads in the garden. <#0.7#>时间放句首搭起重复场景，quietly 靠近 reads，地点留在后面。Fortunately, the team arrived on time. <#0.8#>fortunately 评价的是整件事，并不说明队伍以什么方式到达。
多个状语不能只背“方式、地点、时间”六个字。第一步要看作用范围：有的直接描写动作，有的搭建时间地点背景，有的对整句话表态。<#0.7#>范围越贴近动作的成分，通常越靠近动词；范围较大的背景可以放后面或句首；整句评价常放句首并用逗号隔开。排列后再读一遍，确认每个状语不会错误地粘到旁边成分。中性顺序只是起点，真正决定位置的是它修饰谁、读者先需要哪个背景，以及是否要特别突出某项信息。`,
 'modifier-order':`The red car is mine. <#0.7#>red 很短，直接放在名词 car 前，先给出颜色再说对象。The car parked outside is mine. <#0.7#>parked outside 信息较长，放在 car 后面，说明是哪一辆车。The students who arrived late missed the introduction. <#0.8#>关系从句跟在 students 后，整个长结构一起限定哪些学生。
英语定语的位置主要看形式和重量。单个形容词这类短定语通常放名词前；分词短语、介词短语或从句较长，通常放在名词后，让中心名词先出现。<#0.7#>中文常把说明都放名词前，所以翻译时不能整串照搬。先说中心对象，再把长说明接在后面，读者更容易找到主干。分析长句时先圈出被说明的名词，再把后置部分看成一个整体，别误当成新的主句。判断口诀不是“中文前、英文后”，而是先找中心名词，再看说明结构有多长、属于哪种形式。`,
 'double-object-order':`She gave me a book. <#0.7#>me 是接收者，a book 是被给予的东西；两个都短时，接收者放前面很自然。She gave a book to the new student from Canada. <#0.7#>接收者带着较长说明，于是先放较短的 book，再用 to 引出完整对象。Dad bought a laptop for me. <#0.8#>这里是爸爸为我买，for 表示受益者，不是物品移动的方向。
双宾语表达要先分清两个角色：谁收到或受益，什么东西被传递或准备。常见顺序可以是“接收者加事物”，也可以是“事物加介词短语”。<#0.7#>选择时既看动词允许哪种结构，也看信息重量。接收者很短，可以靠前；接收者较长，放到后面更清楚。to 常指向接收对象，for 常说明为谁做，但不能把所有动词都强行套进这两种转换。先确认动作关系，再选择自然顺序，最后检查介词是否符合这个动词的意义。`,
 'passive-focus':`The storm damaged the bridge. <#0.7#>主动句先说暴风雨，再说它破坏了桥，动作发出者是信息起点。The bridge was damaged by the storm. <#0.7#>被动句把桥提到主语位置，更适合上文正在谈这座桥的情况，真正原因放到后面。The results will be announced tomorrow. <#0.8#>重点是结果和公布时间，谁来公布不重要，所以施事者没有出现。
被动语态不只是把宾语搬到前面，它会重新安排话题和焦点。核心事实仍然是某个动作作用在某个对象上，只是句子选择从承受者讲起。<#0.7#>需要承接旧对象、隐藏未知施事者，或把施事者留作句尾新信息时，被动很合适。若动作发出者本来就是重点，主动句通常更直接。判断时先写清谁做、谁承受，再看上下文正在谈哪一方。语态可以换，角色关系不能换；也不要为了显得正式而无理由使用被动。`,
 'fronting-boundary':`This book I really enjoyed. <#0.7#>this book 虽然放在最前，仍然是 enjoyed 的宾语；前置让它带上对比或承接意味。On the wall hung a large painting. <#0.7#>地点先搭出画面，动词随后出现，新主语留到句尾，像镜头慢慢揭示墙上的东西。Regarding the budget, we need more data. <#0.8#>regarding 明确标记接下来谈预算，主句本身仍有完整主干。
前置属于有标记语序，作用是让某个成分提前成为焦点或话题，不是看到中文话题就随便搬词。<#0.7#>宾语前置后，句法身份不会改变；地点前置有时会配合倒装，把较新的名词主语留后；正式引出话题时，用明确标记比裸放一个名词更清楚。判断边界要看上下文是否真有对比、承接或场景需要。没有这些理由，就回到普通主谓宾顺序。每次前置后还要检查主句是否完整，以及读者能否看出前置成分和哪个位置发生关系。`,
 'inversion-focus':`Never have I seen such a view. <#0.7#>never 被推到句首，have 移到主语前，seen 仍是主要动词；强烈强调过去从未有过。Here comes the bus. <#0.7#>地点提示先出现，新名词主语 the bus 留在句尾，像现场提醒“车来了”。Had I known, I would have acted differently. <#0.8#>had 提到主语前，表达一个没有实现的条件，相当于省略条件连接词的正式结构。
倒装不是把谓语整体随意挪到主语前，而是不同环境有不同移动方式。否定或限制成分前置，通常移动助动词；地点倒装常用于引入新的名词主语，但代词主语一般保持普通顺序；条件倒装只限特定助动词。<#0.7#>看到倒装先找触发点，再分清移动的是助动词还是整个动词，最后确认它是在强调、呈现场景，还是表达正式条件。没有触发条件，就不要为了变化而倒装。`,
 'cleft-focus':`It was Mia who solved the problem. <#0.7#>固定框架把 Mia 放进专门的焦点位置，后面的分句补足其余事实，突出“是米娅”。It was yesterday that we met. <#0.7#>这次焦点换成 yesterday，强调见面的时间，而不是改变见面这件事。What I need is a quiet room. <#0.8#>前半部分先概括“我需要的东西”，be 后面的 quiet room 才是新答案。
强调句像给原句加一束聚光灯。底层事实保持不变，但说话人选择把人物、时间或其他成分照亮。<#0.7#>识别时不要把开头的 it 当普通代词，也不要把后面分句只当普通定语从句；要整体看成焦点框架。另一种分裂结构则把已知或概括内容放前面，把真正答案放在 be 后。使用前先问：上下文真的需要纠正或对比什么吗？如果只是普通叙述，原来的中性句更简洁。强调结构改变的是注意力，不是参与者关系。`,
 'focus-particles':`Only Mia solved the problem. <#0.6#>only 靠着 Mia，排除的是其他人。Mia only solved the problem. <#0.6#>它靠近谓语，重点变成米娅只是解决了问题，没有做更多。Mia solved only the first problem. <#0.6#>这次限制的是宾语范围，只解决第一题。Mia also solved the second problem. <#0.6#>also 表示在已有事实上再增加一项。Even Mia understood the rule. <#0.8#>even 靠着 Mia，传出“连米娅都懂了”的意外感。
only、also、even 都不改变句子主干，却会改变读者怎样理解附近成分。关键不是翻译单词，而是划出它的作用范围。<#0.7#>only 表排除，also 表添加，even 表出乎预期；它们越靠近哪个成分，通常越容易把哪个成分变成焦点。写完后做一个替换测试：我是在限制谁、哪个动作、还是哪个对象？如果换一种位置会产生另一种合理解释，就说明原句需要更清楚的位置或上下文。焦点词虽小，管到的范围却不能含糊。`,
 'negation-scope':`She did not say that he lied. <#0.7#>not 位于主句谓语中，形式上否定的是“她说了这件事”，至于事实怎样，句子没有直接断定。She said that he did not lie. <#0.7#>否定进入后面的从句，明确落在“他撒谎”这个事件上。Not every student passed. <#0.8#>not 和 every 合在一起，意思是并非每个学生都通过，并不等于一个人也没通过。
理解否定不能只看到 not 就翻成“不”。要先看它属于哪个谓语，再判断它覆盖多大范围。<#0.7#>主句里的否定通常先作用于主句事件，从句里的否定作用于从句事件；遇到 every、all、both 这类词，还要分清“并非全部”和“全部都不”。判断时把句子拆成事件层级，再用中文完整说出被否定的部分。若一句话可能让人误会，就把否定词放得更靠近真正目标，或直接改写成明确的部分否定。`,
 'scope-ambiguity':`I did not invite him because he was famous. <#0.7#>这句话可能表示因为他有名，所以我没邀请；也可能表示我邀请了，但原因不是他有名。I did not read every report. <#0.7#>它可能被听成一份也没读，也可能只是没读完全部报告。Not all students found the task easy. <#0.8#>not all 直接把范围限定为“并非所有学生”，意思清楚得多。
复杂否定的问题不在语法，而在 not 可以管到不同范围。原因从句、数量词和较长结构一出现，读者就可能画出两条解释路线。<#0.7#>解决方法不是猜，而是改写。要表达完全没有，就用明确的全无说法；要表达一部分完成，就直接说有些完成但并非全部；要否定原因，就把真实原因或“不是因为”写清楚。改完以后再问：还有没有第二种合理理解？只要答案是有，就继续让否定词贴近目标，直到范围唯一。`,
 'paragraph-progression':`Our school opened a new library. The library has three reading rooms. <#0.7#>第一句末尾的新对象 library，到了下一句开头变成已知主题，读者能顺着它往下走。The library has three reading rooms. It also offers free workshops. <#0.7#>第二组连续使用 the library 和 it，主题保持稳定，再添加新的服务信息。One workshop teaches research skills. These skills help students evaluate sources. <#0.8#>前一句的新内容 research skills，被下一句用 these skills 接住，形成新的推进链。
段落不是许多正确句子的堆叠，而是一条不断交接的话题链。常见做法有两种：保持同一主题，持续补充信息；或者把前一句的新信息变成下一句的起点。<#0.7#>转向子主题也可以，但要用重复词、代词或近义表达搭一座桥。检查段落时只看每句开头：读者能否立刻知道它和上一句的关系？如果主语不断换，却没有承接线索，内容再正确也会显得跳跃。让每句既回望上一句，又向下一句送出新内容，推进就会自然。`,
 'avoid-chinglish':`For this book, I like it very much. <#0.7#>这里开头已经抛出 book，主句又用 it 重复一次，既没有必要，也像把中文话题结构生搬过来。That every student should learn how to evaluate online information is important. <#0.7#>语法成立，但很长的主语堵在前面，核心判断出现得太晚。We discussed the effects of social media on teenagers in modern society. <#0.8#>这句先搭出“我们讨论”，再把复杂宾语按意义一层层组合。
避免中式英语，不是把中文特色全部删掉，而是先换到英语的结构视角。<#0.7#>中文话题要放进主句真实位置，或者用清楚标记引出，不能前面说一次、后面再用代词重复。长内容要考虑后移，让主干尽快出现。翻译复杂意思时，先确定谁做什么，再按名词短语、介词短语和从句分块，不要逐字保持原顺序。写完后遮住中文，只读英文：主干是否完整，修饰关系是否明确，信息是否从轻到重？这三关通过，表达通常就不会带着生硬的逐词痕迹。`,
 'information-rewrite':`A new sports centre opened last week. The centre offers free classes for teenagers. <#0.7#>先用不定表达引入新场馆，再用 the centre 承接，段落推进很清楚。It was announced yesterday that the school would extend the programme. <#0.7#>谁宣布并不重要，被动隐藏施事者；it 又把较长内容放到后面。Only after the survey ended did the team understand the problem. <#0.7#>时间条件被前置成强焦点，因此助动词移到主语前。Not all students found the new system easy to use. <#0.8#>not all 明确表示并非所有人，避免否定范围摇摆。
综合改写不是追求更复杂，而是同时处理承接、重量、焦点和范围。第一步永远是保住事实关系，不能为了换顺序改变谁做什么。<#0.7#>第二步检查新旧信息是否能交接，长内容是否压住主干。第三步才考虑被动、形式成分、前置或倒装，并确认这些有标记结构真的有强调理由。最后专门检查焦点词和否定词管到哪里。能用普通顺序清楚表达，就不必升级结构；只有读者的理解路线确实需要调整时，改写才有意义。`
};
const INFORMATION_ORDER_NARRATION_VERSIONS={'information-order-essence':'v4'};
function narration(id,text){const count=Array.from(text.replace(/<#\d+(?:\.\d+)?#>/g,'').replace(/\s/g,'')).length;return{id:`information-order:${id}`,version:INFORMATION_ORDER_NARRATION_VERSIONS[id]||'v1',text,lengthText:`${count} 字 · 约 2 分钟`};}
function lesson(english,s,index){if(s.rules.length!==s.examples.length||s.rules.length!==s.questions.length)throw new Error(`Information-order coverage mismatch: ${s.id}`);const examples=s.examples.map(x=>({text:x[0].map(c=>c[0]).join(' '),analysis:analysis(english,x[0]),note:note(english,x[1],x[2],x[3])}));const result={id:s.id,no:String(index+1).padStart(2,'0'),level:s.level,title:pick(english,...s.title),meta:pick(english,...s.meta),examples:examples.map(x=>x.text),analyses:examples.map(x=>x.analysis),exampleNotes:examples.map(x=>x.note),rules:s.rules.map(x=>pick(english,...x)),ruleCoverage:INCLUDE_RULE_COVERAGE?s.rules.map((_,i)=>({exampleIndexes:[i],questionIndexes:[i]})):[],questions:s.questions.map((x,i)=>question(english,x,index+i))};const text=INFORMATION_ORDER_NARRATION_TEXTS[s.id];if(text)result.narration=narration(s.id,text);return result;}
